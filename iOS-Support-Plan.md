...
>
> *(Update: editing/checkboxes/Dynamic-Type/keyboard landed in Phase 2b; **syntax highlighting + LaTeX bridges**, then the **GFM tables compositor**, then **horizontally-scrollable wide tables**, were subsequently ported to iOS and verified in the demo. Still out: marked-text/IME, autocorrect lifecycle, context menus beyond the formatting menu.)*

> **RESULT (wide-table horizontal scrolling ported):** the macOS `WideTableOverlay` (`NSScrollView`) now has an iOS twin, `MarkdownTableScrollView` (`UIScrollView`) in `TextView/iOS/`. The styler's wide-vs-narrow decision is unified across platforms again (the iOS-only force-collapsed branch is gone): a table whose natural image width exceeds the text column is emitted as `.collapsedSourceScrollable`, the layout fragment skips drawing it (already cross-platform — `scrollableBlockNaturalWidth` is the cue), and the overlay draws the full image with native horizontal scrolling. On iOS the overlay is a subview of the `MarkdownUITextView` (itself a `UIScrollView`), so it lives in content coords and scrolls vertically with the text; its content overflows only horizontally, so vertical pans propagate to the text view (standard nested horizontal-in-vertical scrolling) while horizontal pans scroll the table. A `MarkdownUITextView.layoutSubviews` width-change hook re-styles so each table's reserved display width tracks rotation/resize; reconcile (create/position/remove + per-`sourceID` offset persistence) runs off `restyleInPlace`. **Verified interactively in the simulator (axe):** the wide table scrolls horizontally while the narrow table beside it doesn't; vertical pans starting on the table scroll the page; tapping the table drops the caret in (→ editable source, overlay removed); tapping out re-renders the image with the overlay restored **at the persisted scroll offset**. macOS inert (156 tests; only the 4 pre-existing `ScrollingHeaderController` flakes red — the styler change restored the original macOS logic verbatim).

> **RESULT (iOS tables ported):** the GFM-table compositor (`MarkdownStyler+Tables.swift`, formerly whole-file `#if os(macOS)`) is now cross-platform. The grid is composited via a new `renderFlippedPlatformImage(size:)` shim — `UIGraphicsImageRenderer` on iOS, the flipped `NSImage` on macOS — so the existing top-down drawing math is unchanged; `NSBezierPath`→`PlatformBezierPath` (+ an `addLineCompat(to:)` line/`addLine` spelling shim), `NSColor`/`NSFont`/`NSFontDescriptor`→`Platform*`, and the muted border/header color now resolves per-platform (UIKit `resolvedColor(with:)` from the threaded scheme; AppKit keeps the `performAsCurrentDrawingAppearance` path **byte-identical**). The `MarkdownStyler.styleTables` call is un-gated. **Wide-table horizontal scroll stays macOS-only** (the `NSScrollView`-backed `WideTableOverlay` isn't ported) — on iOS every table renders collapsed at full width (`.collapsedSource`), a tracked Phase 4 follow-up. Verified in the demo via the simulator: a table with a center-aligned column, **inline bold**, and **inline LaTeX inside a cell** all render correctly in dark mode; macOS inert (156 tests, only the 4 pre-existing `ScrollingHeaderControllerTests` flakes red; "Table cell inline formatting" suite green).
>
> **RESULT (editing layer hardened + reviewed):** after an Opus hate-review and a Codex review cycle (all findings applied — config-change restyle, configured insets, table-LaTeX scheme, vetoed-edit caret), the iOS editing surface was completed: **undo** (edits routed through `UITextInput.replace`, fixing a stale-undo-range crash risk), a **write-back** `onTextChange` (storage-form, no more silent edit loss), **marked-text/IME** guards + re-enabled **autocorrect** (smart-substitutions stay off for Markdown), **blockquote-continued paste**, and **tap-to-open links** (`onLinkTap`). Verified by 7 iOS-simulator editing integration tests + macOS unit tests; macOS inert throughout. The one remaining interaction piece is the **formatting context menu** (Bold/Italic/Heading/List) — see the Phase 4 backlog.
...
| ~~Full-document restyle on iOS (`MarkdownUITextView.restyleInPlace`)~~ | **DONE** — `restyleScoped` + `ParagraphRestyleScoping` port the macOS paragraph-scoping: per-keystroke the edit/selection paths restyle only the caret-neighborhood + edited + active-changed + rendered-block paragraphs (full restyle stays the fallback for loads/config/trait/width and for ``` fence-count changes). Measured on a 13k-char multi-page doc in the simulator: **full restyle 17.2 ms → scoped 1.4 ms (~12×, comfortably under the 16 ms/keystroke budget)**. The whole-document re-parse turned out to be cheap (~0.3 ms) — the cost was the whole-document styling+apply, which scoping removes. 13 scoping unit tests + interactive verification. |
...
- [~] Per-keystroke restyle < 16 ms on a low-end device for a multi-page doc. *(Phase 2b measured ~16.75 ms for a worst-case **full-document** restyle of a ~13k-char doc in the simulator — at budget. **Paragraph-scoped restyle now lands this: 1.4 ms scoped vs 17.2 ms full on the same doc (~12×), simulator.** Remaining open item: a real low-end **device** measurement.)*
