...
| `HighlighterSwiftBridge` / `SwiftMathBridge` (both bridge targets macOS-only) | iOS syntax highlighting + LaTeX — needs color-scheme threaded through `SyntaxHighlighter`/`LatexRenderer` + the fragment, then port off `NSApp`/`NSImage`/`DistributedNotificationCenter` |
